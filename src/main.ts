...
function renderShell() {
  ...
  $('#app').innerHTML=`
    ...
    <main>
      ...
      <section class="sources" id="sources"><div><p class="eyebrow">OPEN COLLECTION, CLEAR ORIGINS</p><h2>${t.sourceTitle}</h2><p>${t.sourceText}</p><small>${t.brandNote}</small></div><div class="source-links"><a href="https://github.com/devicons/devicon" target="_blank" rel="noopener noreferrer">Devicon ${svg('external')}</a><a href="https://github.com/homarr-labs/dashboard-icons" target="_blank" rel="noopener noreferrer">Dashboard Icons ${svg('external')}</a><a href="${file('THIRD_PARTY_NOTICES.md')}" target="_blank" rel="noopener noreferrer">${t.notices} ${svg('external')}</a></div></section>
    </main><footer><span>${t.footer}<small>${t.footerNote}</small></span><div><a href="${file('README.md')}" target="_blank" rel="noopener noreferrer">${t.chineseGuide}</a><a href="${file('README.en.md')}" target="_blank" rel="noopener noreferrer">${t.englishGuide}</a><span>v${catalog.version}</span></div></footer>
...
function showDetail(icon:Icon) {
  ...
  dialog.innerHTML=`<button class="dialog-close" data-close="detail" aria-label="${t.close}">${svg('close')}</button><p class="eyebrow">${t.details}</p><div class="detail-art${dark?' dark-preview':''}"><img src="${file(icon.asset)}" width="96" height="96" alt="${esc(icon.name)}"></div><h2 id="detail-title">${esc(icon.name)}</h2><div class="detail-tags"><span>${esc(title(c))}</span><span>${typeLabel(icon.softwareType)}</span></div><div class="detail-actions"><a class="button primary" href="${file(icon.asset)}" download>${svg('download')}${t.downloadSvg}</a><a class="button outline" href="${file(c.libraries[locale])}" download>${t.downloadLibrary}</a></div>
    <dl><dt>${t.project}</dt><dd><a href="${esc(icon.homepage)}" target="_blank" rel="noopener noreferrer">${t.project} ↗</a></dd>${icon.repository?`<dt>${t.repository}</dt><dd><a href="${esc(icon.repository)}" target="_blank" rel="noopener noreferrer">${t.repository} ↗</a></dd>`:''}<dt>${t.source}</dt><dd><a href="${esc(icon.source.url)}" target="_blank" rel="noopener noreferrer">${icon.source.id==='devicon'?'Devicon':'Dashboard Icons'} ↗</a></dd><dt>${t.license}</dt><dd><a href="${esc(icon.source.licenseUrl)}" target="_blank" rel="noopener noreferrer">${esc(icon.source.collectionLicense)} ↗</a></dd><dt>${t.revision}</dt><dd><code>${esc(icon.source.revision.slice(0,12))}</code></dd></dl><div class="detail-notes"><p>${t.typeNote}</p><p>${t.brandNote}</p></div>`;
  dialog.showModal();
}
function showBundle() {
