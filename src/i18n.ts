...
export const messages = {
  'zh-CN': {
    ...
    downloadPng:'下载 PNG',officialPublisher:'官方发布者',publisher:'发布者',rightsNotice:'图标权利说明',contentPin:'清单校验值',officialRaster:'官方发布的 512 × 512 彩色 PNG 原图；钉钉使用 SVG 包装添加圆角透明裁切，内部原图不变。图片在 draw.io 库中内嵌，放大不等同于矢量图。',
    library:'图标库',all:'全部分类',categoryLabel:'分类',bundle:'选择分类加载 / 桌面配置',bundleHint:'跨图标集选分类，打开网页版或合并桌面配置',
    openAll:'加载{collection}图标到 draw.io',homeLoadHint:'第一个按钮始终加载全部合集，不受左侧图标集、分类或搜索筛选影响。需要部分分类或桌面端配置，请使用「选择分类加载 / 桌面配置」。',alibabaLoadHint:'默认加载全部阿里云分类；勾选后加载所有图标集。首页加载范围独立于左侧浏览筛选。',
    ...
    openDrawio:'加载到 draw.io',downloadLibrary:'下载分类库',allDescription:'从基础设施到开发工具，找到你的技术栈。',allLibrary:'下载全集 XML',
  ...
  en: {
    ...
    downloadPng:'Download PNG',officialPublisher:'Official publisher',publisher:'Publisher',rightsNotice:'Artwork rights notice',contentPin:'Manifest checksum',officialRaster:'Official 512 × 512 color PNG artwork. DingTalk adds an SVG rounded-corner clip around the unchanged PNG. Artwork is embedded in the draw.io library and remains raster when enlarged.',
    library:'LIBRARY',all:'All categories',categoryLabel:'CATEGORY',bundle:'Choose categories / desktop setup',bundleHint:'Choose across collections; open on the web or merge desktop settings',
    openAll:'Load {collection} icons into draw.io',homeLoadHint:'The first button always loads every collection, regardless of sidebar or search filters. Use “Choose categories / desktop setup” for selected categories or desktop configuration.',alibabaLoadHint:'Load every Alibaba Cloud category by default. Check the box to include all collections. Homepage loading is independent of sidebar filters.',
    ...
    openDrawio:'Load into draw.io',downloadLibrary:'Download library',allDescription:'From infrastructure to developer tools. Find your stack.',allLibrary:'Download all XML',
