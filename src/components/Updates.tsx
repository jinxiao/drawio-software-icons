...
function UpdateRecord({entry,initialOpen}:{entry:UpdateEntry;initialOpen:boolean}) {
  ...
  })}</div>{!group.icons.length&&<><p>{t.collectionImport}</p><Button variant="outline" size="sm"  data-update-collection={group.collection} onClick={()=>{s.selectCollection(group.collection,true);history.replaceState(null,'',location.pathname+location.search+'#library');s.setFocusRequest({kind:'heading',scroll:true});}}>{t.browse}</Button></>}</div>)}{entry.commit&&<a className="update-commit" href={`https://github.com/jinxiao/drawio-software-icons/commit/${entry.commit}`} target="_blank" rel="noopener noreferrer">{t.viewChange}<Symbol name="external"/></a>}</div></details>;
}
