...
export function SpotlightDialog() {
  ...
  return <Dialog id="spotlight" label={t.quickSearch} onClose={s.closeModal}>
    ...
    }}/><button className="spotlight-close" onClick={s.closeModal} aria-label={t.close}><kbd>Esc</kbd></button></div>
...
export function DetailDialog({icon}:{icon:Icon}) {
  ...
  return <Dialog id="detail" labelledBy="detail-title" onClose={s.closeModal}><CloseButton onClose={s.closeModal}/><p className="eyebrow">{t.details}</p><div className={`detail-art${dark?' dark-preview':''}`}><img src={file(icon.asset)} width="96" height="96" alt={s.iconName(icon)}/></div><h2 id="detail-title">{s.iconName(icon)}</h2><div className="detail-tags"><span>{s.title(category)}</span><span>{s.typeLabel(icon.softwareType)}</span></div><div className="detail-actions"><a className="button primary" href={file(icon.asset)} download><Symbol name="download"/>{icon.asset.endsWith('.png')?t.downloadPng:t.downloadSvg}</a><a className="button outline" href={file(category.libraries[locale])} download>{t.downloadLibrary}</a></div>
...
export function LocalNotice() {
  ...
  return <Dialog id="notice" labelledBy="notice-title" onClose={closeModal}><CloseButton onClose={closeModal}/><h2 id="notice-title">{t.navGuide}</h2><p id="notice-body">{t.localNote}</p><a className="button primary" href={file('downloads/drawio-icons.zip')} download>{t.downloadAll}</a></Dialog>;
}
