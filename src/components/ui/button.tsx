  ...
  "inline-flex min-w-0 max-w-full shrink-0 cursor-pointer items-center justify-center gap-2 rounded-md border border-solid border-transparent bg-transparent p-0 text-sm leading-normal font-medium text-foreground whitespace-normal [overflow-wrap:anywhere] transition-all outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 motion-reduce:transition-none [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        ...
        outline:
          "border-input bg-card shadow-xs hover:bg-accent hover:text-accent-foreground",
        secondary:
          "bg-secondary text-secondary-foreground hover:bg-secondary/80",
        ghost:
          "hover:bg-accent hover:text-accent-foreground aria-pressed:bg-secondary aria-pressed:text-secondary-foreground",
      ...
      size: {
        ...
        "icon-sm": "size-8",
        "icon-lg": "size-10",
        // Search triggers and icon cards keep their content-driven layout.
        layout: "h-auto",
      },
    },
    defaultVariants: {
...
function Button({
  ...
  return (
    <Comp
      ...
      {...props}
    >
      {asChild ? children : <>{loading && <LoaderCircle className="size-4 animate-spin motion-reduce:animate-none" aria-hidden="true" />}{children}</>}
    </Comp>
  )
}

function ButtonLink({ className, variant = "default", size = "default", ...props }:
  React.ComponentProps<"a"> & VariantProps<typeof buttonVariants>) {
  return <a data-slot="button" data-variant={variant} data-size={size}
