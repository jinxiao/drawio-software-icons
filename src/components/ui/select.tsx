...
function SelectTrigger({ className, children, ...props }: React.ComponentProps<typeof SelectPrimitive.Trigger>) {
  return <SelectPrimitive.Trigger data-slot="select-trigger"
    className={cn('flex h-auto min-h-9 w-full min-w-0 cursor-pointer items-center justify-between gap-2 rounded-md border border-solid border-input bg-card px-3 py-2 text-sm leading-normal text-foreground shadow-xs outline-none transition-colors hover:bg-muted focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/30 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:min-w-0 [&>span]:text-left [&>span]:whitespace-normal [&>span]:[overflow-wrap:anywhere]', className)} {...props}>
    {children}
...
function SelectContent({ className, children, container, ...props }: React.ComponentProps<typeof SelectPrimitive.Content> & {
  ...
  return <SelectPrimitive.Portal container={container}>
    <SelectPrimitive.Content data-slot="select-content" position="popper" align="start" sideOffset={5} collisionPadding={12}
      className={cn('relative z-50 max-h-(--radix-select-content-available-height) w-(--radix-select-trigger-width) max-w-[calc(100vw-24px)] overflow-hidden rounded-md border border-solid border-border bg-card text-foreground shadow-md', className)} {...props}>
...
function SelectItem({ className, children, ...props }: React.ComponentProps<typeof SelectPrimitive.Item>) {
  return <SelectPrimitive.Item data-slot="select-item"
    className={cn('relative flex min-h-9 w-full min-w-0 cursor-pointer items-center rounded-sm py-2 pr-8 pl-3 text-sm leading-normal whitespace-normal [overflow-wrap:anywhere] outline-none select-none focus:bg-accent focus:text-accent-foreground data-[state=checked]:bg-secondary data-[state=checked]:font-medium data-[disabled]:pointer-events-none data-[disabled]:opacity-50', className)} {...props}>
