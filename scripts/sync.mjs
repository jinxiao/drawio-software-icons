...
async function download(url) {
  const cache = `.sync-stage/cache/${hash(url)}.txt`;
  const cached = await readFile(cache,'utf8').catch(e=>{if(e.code!=='ENOENT')throw e;return null;});
...
async function worker() {
  while (cursor < selection.length) {
    ...
    try {
      ...
      let raw;
      if (prior?.source.url === sourceUrl) {
        try { raw = await readFile(`assets/icons/${item.id}.svg`,'utf8'); if(hash(raw)!==prior.sha256) raw = null; }
      ...
      await save(`${stage}/assets/icons/${item.id}.svg`,raw);
